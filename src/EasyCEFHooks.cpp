...
PVOID origin_cef_browser_host_create_browser_async = nullptr;
...
int CEF_CALLBACK hook_cef_on_key_event(struct _cef_keyboard_handler_t* self,
	...
	if (!origin_cef_on_key_event) return 0;
...
struct _cef_keyboard_handler_t* CEF_CALLBACK hook_cef_get_keyboard_handler(struct _cef_client_t* self) {
	if (!origin_cef_get_keyboard_handler) return nullptr;
...
struct _cef_load_handler_t* CEF_CALLBACK hook_cef_load_handler(struct _cef_client_t* self) {
	if (!origin_cef_load_handler) return nullptr;
...
_cef_display_handler_t* CEF_CALLBACK hook_cef_client_get_display_handler(_cef_client_t* self) {
	if (!origin_cef_client_get_display_handler) return nullptr;
	auto* display_handler = CAST_TO(origin_cef_client_get_display_handler, hook_cef_client_get_display_handler)(self);
	if (!display_handler) return nullptr;
	display_handler->on_title_change = [](struct _cef_display_handler_t*, struct _cef_browser_t* browser, const cef_string_t* title) {
		if (!browser || !title) return;
		auto frame = browser->get_main_frame(browser);
		if (frame && frame->get_url && !BNString(util::cefFromCEFUserFreeTakeOwnership(frame->get_url(frame)).ToWString()).startsWith(L"devtools://")) {
			auto host = browser->get_host(browser); if (host) { auto hwnd = host->get_window_handle(host); if (hwnd) SetWindowText(hwnd, std::wstring(util::cefFromCEFUserFree(title)).c_str()); }
		}
...
cef_browser_t* hook_cef_browser_host_create_browser_sync(const cef_window_info_t* windowInfo, struct _cef_client_t* client, const cef_string_t* url, const struct _cef_browser_settings_t* settings, struct _cef_dictionary_value_t* extra_info, struct _cef_request_context_t* request_context) {
	prepare_client_hooks(client); if (!origin_cef_browser_host_create_browser) return nullptr;
	return CAST_TO(origin_cef_browser_host_create_browser, hook_cef_browser_host_create_browser_sync)(windowInfo, client, url, settings, extra_info, request_context);
}

int hook_cef_browser_host_create_browser(const cef_window_info_t* windowInfo, struct _cef_client_t* client, const cef_string_t* url, const struct _cef_browser_settings_t* settings, struct _cef_dictionary_value_t* extra_info, struct _cef_request_context_t* request_context) {
	prepare_client_hooks(client); if (!origin_cef_browser_host_create_browser_async) return 0;
	return CAST_TO(origin_cef_browser_host_create_browser_async, hook_cef_browser_host_create_browser)(windowInfo, client, url, settings, extra_info, request_context);
}
...
void CEF_CALLBACK hook_on_context_created(struct _cef_render_process_handler_t* self, struct _cef_browser_t* browser, struct _cef_frame_t* frame, struct _cef_v8context_t* context) {
	if (frame && frame->get_url && BNString(util::cefFromCEFUserFreeTakeOwnership(frame->get_url(frame)).ToWString()).startsWith(L"orpheus://")) process_context(context);
	if (origin_cef_app_on_context_created) CAST_TO(origin_cef_app_on_context_created, hook_on_context_created)(self, browser, frame, context);
}
void CEF_CALLBACK hook_on_context_released(struct _cef_render_process_handler_t* self, struct _cef_browser_t* browser, struct _cef_frame_t* frame, struct _cef_v8context_t* context) {
	if (origin_cef_app_on_context_released) CAST_TO(origin_cef_app_on_context_released, hook_on_context_released)(self, browser, frame, context);
}

struct _cef_render_process_handler_t* CEF_CALLBACK hook_get_render_process_handler(struct _cef_app_t* self) {
	if (!origin_get_render_process_handler) return nullptr; auto* handler = CAST_TO(origin_get_render_process_handler, hook_get_render_process_handler)(self); if (!handler) return nullptr;
	origin_cef_app_on_context_created = handler->on_context_created; handler->on_context_created = hook_on_context_created; origin_cef_app_on_context_released = handler->on_context_released; handler->on_context_released = hook_on_context_released; return handler;
}
int hook_cef_execute_process(const struct _cef_main_args_t* args, cef_app_t* application, void* windows_sandbox_info) {
	if (!origin_cef_execute_process || !application) return -1; if (application->get_render_process_handler) { origin_get_render_process_handler = application->get_render_process_handler; application->get_render_process_handler = hook_get_render_process_handler; } return CAST_TO(origin_cef_execute_process, hook_cef_execute_process)(args, application, windows_sandbox_info);
}
int hook_cef_initialize(const struct _cef_main_args_t* args, const struct _cef_settings_t* settings, cef_app_t* application, void* windows_sandbox_info) {
	if (!origin_cef_initialize || !settings || !application) return 0; _cef_settings_t s = *settings; s.background_color = 0x000000ff; if (application->on_before_command_line_processing) { origin_on_before_command_line_processing = application->on_before_command_line_processing; application->on_before_command_line_processing = hook_on_before_command_line_processing; } if (application->get_render_process_handler) { origin_get_render_process_handler = application->get_render_process_handler; application->get_render_process_handler = hook_get_render_process_handler; } return CAST_TO(origin_cef_initialize, hook_cef_initialize)(args, &s, application, windows_sandbox_info);
}
...
_cef_resource_handler_t* CEF_CALLBACK hook_cef_scheme_handler_create(
	...
	if (!origin_cef_scheme_handler_create || !request) return nullptr;
...
int hook_cef_register_scheme_handler_factory(
	...
	if (!origin_cef_register_scheme_handler_factory || !factory || !factory->create) return 0;
...
void EasyCEFHooks::executeJavaScript(_cef_frame_t* frame, const std::string& script, const std::string& url) {
	if (!frame || !frame->execute_java_script) return;
