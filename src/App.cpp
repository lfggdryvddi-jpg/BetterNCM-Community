...
constexpr std::string version = "1.5.0";
...
std::thread* App::create_server(const std::string& apiKey) {
	...
	if (this->server_port <= 0) {
		...
		throw std::runtime_error("Failed to bind BetterNCM local API");
	}
...
void App::Init() {
	...
	EasyCEFHooks::onKeyEvent = [](_cef_client_t* client, struct _cef_browser_t* browser,
		...
		if (event->type == KEYEVENT_KEYUP &&
			...
			if (frame && frame->get_url && BNString(cefFromCEFUserFreeTakeOwnership(frame->get_url(frame)).ToWString()).startsWith(
				L"devtools://")) {
	...
	EasyCEFHooks::onLoadStart = [=](_cef_browser_t* browser, _cef_frame_t* frame) {
		CefString url;
		if (frame && frame->get_url) url = cefFromCEFUserFreeTakeOwnership(frame->get_url(frame));

		if (browser && frame && frame->is_main && frame->is_valid && frame->is_main(frame) && frame->is_valid(frame)) {
