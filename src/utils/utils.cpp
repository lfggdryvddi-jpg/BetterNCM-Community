...
semver::version util::getNCMExecutableVersion() {
	static const semver::version fallback{"0.0.0"};
	static std::optional<semver::version> cached;
	if (cached.has_value()) return cached.value();

	const auto executable = getNCMPath() + L"\\cloudmusic.exe";
	DWORD verHandle = 0;
	const DWORD verSize = GetFileVersionInfoSizeW(executable.c_str(), &verHandle);
	if (verSize == 0) {
		cached = fallback;
		return cached.value();
	}
	...
	UINT size = 0;
	if (VerQueryValueW(verData.data(), L"\\", &lpBuffer, &size) && size >= sizeof(VS_FIXEDFILEINFO)) {
		const auto* verInfo = static_cast<const VS_FIXEDFILEINFO*>(lpBuffer);
		if (verInfo->dwSignature == 0xfeef04bd) {
			cached = semver::version{
				static_cast<uint8_t>((verInfo->dwFileVersionMS >> 16) & 0xffff),
				static_cast<uint8_t>(verInfo->dwFileVersionMS & 0xffff),
